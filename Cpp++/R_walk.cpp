...
// 	if (exp % 2 != 0) {vvi multiply(vvi &a, vvi &b) {
// 	int n = a.size();
// 	vvi ans(n, vector<ll>(n, 0));
// 	loop(i, 0, n - 1) {
// 		loop(j, 0, n - 1) {
...
vii  Multiply(vii &a, vii &b) {
	ll n = a.size();
	vii ans(n, vector<ll>(n, 0));
	loop(i, 0, n - 1) {
		loop(j, 0, n - 1) {
			loop(k, 0, n - 1) {
				ans[i][j] = (ans[i][j] % mod + (a[i][k] * a[k][j]) % mod) % mod;
			}
		}
	}
	return ans;
...
vii power_(vii &mat, ll exp) {
	if (exp == 1)return mat;
	vii ans = power_(mat, exp / 2);
	vii res = Multiply(ans, ans);
	if (exp % 2 != 0) {
